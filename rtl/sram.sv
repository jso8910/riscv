...
module sram #(
    ...
    input logic clk,
    input logic [AWIDTH-1:0] address_i,
    ...
    always_comb begin
        ...
        for (int i = 0; i < NUM_BYTES; i++) begin
            mem_byte_array[i] = (address_i + i <= END_ADDRESS) && (address_i + i >= START_ADDRESS) ? mem[address_i + i] : 8'b0;
            data_o[i * 8 +: 8] = mem_byte_array[i];
        end
    end
    ...
    always_ff @(posedge clk) begin
        ...
        // segment of memory).
        for (int i = 0; i < NUM_BYTES; i++) begin
            if (we_i[i] == 1'b1 && (address_i + i <= END_ADDRESS) && (address_i + i >= START_ADDRESS)) begin
                mem[address_i + i] <= data_i[i * 8 +: 8];
            end
        end
    end
endmodule : sram
